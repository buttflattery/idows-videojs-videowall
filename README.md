...
|----------         |-------------------|     ------------------|
|`callback`         |`null`         | `$('#id').idowsVideoWall({`<br />`callback:function(){`<br />`console.log('finished loading);`<br />`}`<br />`});`<br />Description:<br/><br />__A callback function that is triggered after the initialization of the plugin, called once/instance.__
